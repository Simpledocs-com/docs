...
Check the `optional_fields` array in the workflow schema response before submitting — it lists what the workflow accepts. `support_documents[]` is always available. Some optional fields, such as `merge_documents_into_pdf`, only appear when enabled for your account — contact support to request access.

- `support_documents[]` accepts one or more file uploads.
- `merge_documents_into_pdf` must be `"true"` to enable merging supporting documents into the main document; any other value (including omitting the field) is treated as false.
